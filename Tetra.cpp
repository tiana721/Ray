...
int Tetra::intersect(const Ray& ray, std::vector<double>& point) const
{
    ...
    if ((dist[0] >= 1e-5) || (dist[1] >= 1e-5) || (dist[2] >= 1e-5) || (dist[3] >= 1e-5)) {
        for (int i = 0; i < 4; i++) {
...
{ 
    ...
    int s1 = 0, s2 = 0, s3 = 0, s4 = 0, res_count = 0;
    ...
    if ((numbers[0] == 1) || (numbers[1] == 1) || (numbers[2] == 1)) {
        vertex.push_back(vert1); v1 = 1;
    }
    if ((numbers[0] == 2) || (numbers[1] == 2) || (numbers[2] == 2)) {
        vertex.push_back(vert2); v2 = 1;
    }
    ...
    vector<double> new_pnt = {0, 0, 0}, curvert;
    int new_inter = 0;
    double dot = 0;
    ...
    //dot = (norm.at(0) * toLight.get_dirX() + norm.at(1) * toLight.get_dirY() + norm.at(2) * toLight.get_dirZ());
    if (product <= 0)
        return RGB(0, 0, 0);
